...
const PUBLIC_PATHS = [
  ...
  '/allergens',
  // Reverse ingredient search ("What can I make?"), page and API. Read-only,
  // rate-limited by searchLimiter on top of the global /api floor above. Public
  // on purpose: a visitor has to be able to try it before signing up, which is
  // the entire funnel this feature exists to feed.
  '/what-can-i-make',
  '/api/search',
]
